...
class RenderedValuesCache(object):
    ...
    def get_value_generators(self):
        return self._value_generators

    def set_value_generators(self, value_generators):
...
class Request(object):
        ...
        for request_block in req_definition:
            ...
            # Handle custom (user defined) payloads
                ...
                try:
                    ...
                    elif isinstance(current_fuzzable_values, types.FunctionType) and\
                         current_fuzzable_values.__name__ == primitives.VALUE_GENERATOR_WRAPPER_FUNC_NAME:
            ...
            # Handle all the rest
                ...
                if isinstance(values, types.FunctionType) and\
                         values.__name__ == primitives.VALUE_GENERATOR_WRAPPER_FUNC_NAME:
    ...
    def render_iter(self, candidate_values_pool, skip=0, preprocessing=False, prev_rendered_values=None):
        ...
        def init_value_generators(fuzzable_request_blocks, fuzzable, value_gen_tracker):
            ...
            for idx in fuzzable_request_blocks:
                ...
                and isinstance(fuzzable[idx][0][0], types.FunctionType)\
                and fuzzable[idx][0][0].__name__ == primitives.VALUE_GENERATOR_WRAPPER_FUNC_NAME:
        ...
        for req in self.get_schema_combinations():
            ...
            value_generators = self._rendered_values_cache.get_value_generators()
            value_gen_tracker = self._rendered_values_cache.value_gen_tracker
            if value_generators is None:
                value_generators = init_value_generators(fuzzable_request_blocks, fuzzable, value_gen_tracker)
                self._rendered_values_cache.set_value_generators(value_generators)
    ...
    def render_current(self, candidate_values_pool, preprocessing=False, use_last_cached_rendering=False):
        ...
        rendered_values, parser, tracked_parameter_values = \
            next(self.render_iter(candidate_values_pool,
                                  skip=self._current_combination_id - 1,
                                  preprocessing=preprocessing,
                                  prev_rendered_values=prev_rendered_values))
        return rendered_values, parser, tracked_parameter_values
